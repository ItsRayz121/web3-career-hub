...
export default function TrackerPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    await supabase.from('saved_opportunities').update({ status, updated_at: new Date().toISOString() }).eq('id', id)
  ...
  const counts = statuses.reduce((acc, s) => {
    acc[s] = opportunities.filter(o => o.status === s).length
    return acc
        ...
        {statuses.map(s => (
          <button
            ...
            className={`rounded-lg p-3 text-center border transition-all ${
              filterStatus === s ? 'border-[#6c63ff] bg-[#6c63ff10]' : 'border-[#1e1e35] bg-[#12121f] hover:border-[#6c63ff30]'
            }`}
          >
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
            filterStatus === 'all' ? 'bg-[#6c63ff] text-white' : 'bg-[#1a1a2e] text-[#8888aa] hover:text-white border border-[#1e1e35]'
          }`}
        >
        ...
        {statuses.map(s => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all ${
              filterStatus === s ? 'bg-[#6c63ff] text-white' : 'bg-[#1a1a2e] text-[#8888aa] hover:text-white border border-[#1e1e35]'
            }`}
          >
            {s} ({counts[s] || 0})
          </button>
            ...
            <Card key={opp.id} className="group">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {opp.deadline && (
                    <p className="text-xs text-yellow-400 mt-0.5">Deadline: {formatDate(opp.deadline)}</p>
                  )}
                  ...
                  <textarea
                    ...
                    onBlur={e => updateNotes(opp.id, e.target.value)}
                  />
                </div>
